...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $policyNumber = str_replace('<', '&lt' , $policyNumber);
        $policyNumber = str_replace('>' ,'&gt' , $policyNumber);

        $policyDate = $_POST['policyDate'];
        $policyDate = str_replace('<', '&lt' , $policyDate);
        $policyDate = str_replace('>' ,'&gt' , $policyDate);

        $policyAmount = $_POST['policyAmount'];
        $policyAmount = str_replace('<' , '&lt' , $policyAmount);
        $policyAmount = str_replace('>' , '&gt' , $policyAmount);

        $policyMaturityDate = $_POST['policyMaturity'];
        $policyMaturityDate = str_replace('<' , '&lt' , $policyMaturityDate);
        $policyMaturityDate = str_replace('>' , '&gt' , $policyMaturityDate);

        $policyTerm = $_POST['policyTerm'];
        $policyTerm = str_replace('<' , '&lt' , $policyTerm);
        $policyTerm = str_replace('>' , '&gt' , $policyTerm);

        $policyPremium = $_POST['policyPremium'];
        $policyPremium = str_replace('<' , '&lt' , $policyPremium);
        $policyPremium = str_replace('>' , '&gt' , $policyPremium);

        $premiumTime = $_POST['premiumTime'];
        $premiumTime = str_replace('<' , '&lt' , $premiumTime);
        $premiumTime = str_replace('>' , '&gt' , $premiumTime);

        $nomineeName = $_POST['nomineeName'];
        $nomineeName = str_replace('<' , '&lt' , $nomineeName);
        $nomineeName = str_replace('>' , '&gt' , $nomineeName);

        $nomineeRelation = $_POST['nomineeRelation'];
        $nomineeRelation = str_replace('<' , '&lt' , $nomineeRelation);
        $nomineeRelation = str_replace('>' , '&gt' , $nomineeRelation);

        $nomineeDob = $_POST['nomineeDob'];
        $nomineeDob = str_replace('<' , '&lt' , $nomineeDob);
        $nomineeDob = str_replace('>' , '&gt' , $nomineeDob);
        ...
        if ($nums > 0) {
            echo '<div class="alert alert-danger alert-dismissible fade show body2" role="alert">
            <strong>Policy Number Alreday Exits!!</strong>
          </div>';
        } else {
            $sql = "INSERT INTO `plc_mst` (`name` , `policyNumber` , `policyDate` , `policyAmount` , `policyMaturityDate` , `policyTerm` , `policyPremium` , `premiumTime` , `nomineeName` , `nomineeRelation` , `nomineeDob` ,`id`, `date`) VALUES ('$name','$policyNumber' , '$policyDate' , '$policyAmount' , '$policyMaturityDate' , '$policyTerm' , '$policyPremium' , '$premiumTime' , '$nomineeName' , '$nomineeRelation' , '$nomineeDob' ,'$id', current_timestamp())";
            ...
            <div class="container bodyDiv my-4">
                <form class="needs-validation" novalidate="" action="/mypolicy/policy.php?sno=<?php echo $sno ?>" method="post">
                    <div class="col-md-12 ">
                        ...
                        <select class="form-control form-control-sm" id="name" name="name" required>
                            ...
                            <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                                <option value="<?php echo $row['sno']; ?>-<?php echo $row['firstname']; ?>"><?php echo $row['sno']; ?>-<?php echo $row['firstname']; ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>

                    <input type="number" name="id" id="id" value="<?php echo $sno ?>" hidden>
                    ...
                    <div class="col-sm-12">
                        ...
                        <select class="form-control" id="premiumTime" name="premiumTime" required>
                    ...
                    <div class="col-sm-12">
                        ...
                        <select class="form-control" id="nomineeRelation" name="nomineeRelation" required>
                            <option value="">Select option</option>
